...
export async function uploadBuffer(
  ...
  const cloud_storage_path = isPublic
    ...
    : `${folderPrefix}uploads/${Date.now()}-${fileName}`
  await s3.send(
    new PutObjectCommand({ Bucket: bucketName, Key: cloud_storage_path, Body: buffer, ContentType: contentType }),
  )
  return { cloud_storage_path }
}

export async function getFileUrl(cloud_storage_path: string, contentType = 'image/png', isPublic = true) {
